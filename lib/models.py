        ...
        with tqdm(total=num_steps) as pbar:
            for step in range(1, num_steps+1):
                ...
                if step % self.eval_frequency == 0 or step == num_steps:
                    ...
                    pbar.set_postfix(learning_rate='{:.3e}'.format(learning_rate),
                                     loss_avg='{:.3e}'.format(loss_average),
                                     validation_loss='{:.3e}'.format(loss))
...
#                  pool='mpool1', num_epochs=20, learning_rate=0.1, decay_rate=0.95, decay_steps=None, momentum=0.9,
...
#         if p > 1:
#             x = tf.expand_dims(x, 3)  # N x M x F x 1
#             x = tf.image.resize_images(x, size=[int(M*p), int(Fin)])  # , method=tf.image.ResizeMethod.BICUBIC)  # NxMxF
